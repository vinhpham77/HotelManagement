...
export class CustomersComponent implements OnDestroy, OnInit, AfterViewInit {
  ...
  displayedColumns: string[] = ['select', 'firstName', 'lastName', 'birthdate', 'sex', 'idNo', 'phoneNumber', 'address', 'nationality', 'actions'];
  ...
  onDelete(customer: Customer) {
    this.customersService.delete(customer.id).subscribe(() => {
      this.commonService.openSnackBar('Xoá thành công khách hàng ' + customer.firstName);
      this.loadCustomers();
    });
  }
  ...
  onDeleteMany() {
    ...
    this.customersService.deleteMany(customerIds).subscribe(() => {
      this.commonService.openSnackBar('Xoá thành công ' + customerIds.length + ' khách hàng');
      this.loadCustomers();
    });
  }
